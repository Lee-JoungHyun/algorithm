                ...
                for (int x = 0; x < N; x++) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        if (0 <= y+dy[i] && y+dy[i] < N && 0 <= x+dx[i] && x+dx[i] < N && !visited[y+dy[i]][x+dx[i]]) {
                            if (L <= Math.abs(map[y][x] - map[y+dy[i]][x+dx[i]]) &&
                                    Math.abs(map[y][x] - map[y+dy[i]][x+dx[i]]) <= R){
                                flag = false;
                                break;
                            }
                        }
                    ...
                    while (queue.size() != 0) {
                        ...
                        for (int i = 0; i < 4; i++) {
                            nxtY = now[0]+dy[i];
                            nxtX = now[1]+dx[i];
                            if (0 <= nxtY && nxtY < N && 0 <= nxtX && nxtX < N && !visited[nxtY][nxtX] &&
                                    L <= Math.abs(map[now[0]][now[1]] - map[nxtY][nxtX]) &&
                                    Math.abs(map[now[0]][now[1]] - map[nxtY][nxtX]) <= R ) {
